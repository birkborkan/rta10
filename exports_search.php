            ...
            if(empty($s_date) and empty($sch_by))
             ...
             echo"<p class='add'><script> document.write('الرجاء كتابة نص للبحث عنه') </script></p>";
                 ...
                 { 
                    ...
                    if($sch_type=='rname')
                        {    $rep_type="صادر لمرحل:";
                          $rep_type_value=$sch_by;
                          $rep_date=" -- بتاريخ:".$date;
     $q=mysql_query(" select * from store_exp where rname like '%$sch_by%' 
      and  ldate like '$s_date' order by eid desc");
                        }
                       else if($sch_type=='dname')
                        {   $rep_type="صادر لسائق:";
                          $rep_type_value=$sch_by;
                          $rep_date=" -- بتاريخ:".$date;
     $q=mysql_query(" select * from store_exp where dname like '%$sch_by%'  and 
        ldate like '$s_date' order by eid desc");
                        }
                        else if($sch_type=='lno')
                        {
                          $rep_type="صادر لعربة برقم :";
                          $rep_type_value=$sch_by;
                          $rep_date=" -- بتاريخ:".$date;
$q=mysql_query(" select * from store_exp where cno like '%$sch_by%'  and 
 ldate like '$s_date' order by eid desc");
                        }
                        ...
                        {   $rep_type="صادر لوحدة:";
                          $rep_type_value=$sch_by;
                          $rep_date=" -- بتاريخ:".$date;
       $q=mysql_query(" select * from store_exp where pname like '$sch_by' and 
ldate like '$s_date' order by eid desc");
                       } 
                        ...
                        {     $rep_type="صادر لصنف:";
                          $rep_type_value=$sch_by;
                          $rep_date=" -- بتاريخ:".$date; 
 $q=mysql_query(" select * from store_exp where ptype like '%$sch_by%' and 
ldate like '$s_date' order by eid desc");
                       }
                }
...
<?php 
      ...
      echo "<div  class='rep_title2'>".$rep_type." ".$rep_type_value."".$rep_date." </div>";   
